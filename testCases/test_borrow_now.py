...
class Test_002_BorrowNow:
    ...
    def test_borrowNow(self, setup):
        ...
        time.sleep(3)
        self.text = self.borrowNow.getFirstItemText()

        try:
            # Code that might raise an assertion error
            if not self.text == self.firstItemText:  # Replace 'condition' with your actual condition
            ...
            error_details = f"{datetime.datetime.now()}: AssertionError: First Item Details Did not Match"
